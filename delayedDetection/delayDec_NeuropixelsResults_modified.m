function delayDec_NeuropixelsResults_modified(Animal)
...
elseif strcmpi(Animal, 'N14')
    ...
    depthClusters{1} = {[2011 3700], [871 2010], [0 870]};
end
preStim = 0.5;
stimDur = 2.5;
sRate = 30;
...
for x = 1 : length(fPath)
    ...
    for y = 1:size(allSpikes{x},2)
        % modification 10/19/22
        %cFull{x}(y,1) = corr2(allSpikes{x}(:,y), fullV(:,y))^2;  %%
        cFull{x}(y,1) = VE(allSpikes{x}(:,y), fullV(:,y));
        for z = 1:size(allV,4)
            % modification 10/19/22
%             cRegs{x}(y,1,z) = corr2(allSpikes{x}(:,y), allV(:,y,1,z))^2;
%             cRegs{x}(y,2,z) = corr2(allSpikes{x}(:,y), allV(:,y,2,z))^2;
            cRegs{x}(y,1,z) = VE(allSpikes{x}(:,y), allV(:,y,1,z));
            cRegs{x}(y,2,z) = VE(allSpikes{x}(:,y), allV(:,y,2,z));
        end
    end
end
cFull = cat(1,cFull{:});
